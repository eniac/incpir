...
void refresh_query(NetClient& client, Database& my_database, int db_size, int id, vector<Block>&query_res, OnlineQuery& online_qry, string ip, Block& blk) {
    double total_time = 0.0;
    client.refresh_query(ip, online_qry, total_time, blk);
...
int main(int argc, char *argv[]) {
    ...
    double tmp_time = 0.0;
    Block blk = client.online_query(ip, online_qry, tmp_time);

    for (int i = 0; i < offer_load; i++) {   
        OnlineQuery refresh_query = client.generate_refresh_query(qry_idx);
        querys[i] = refresh_query;
    }
    system_clock::time_point start, end;
    start = system_clock::now();
    for (int i = 0; i < offer_load; i++) {
        thread t(&refresh_query, ref(client), ref(my_database), db_size, i, ref(query_res), ref(querys[i]), ip, ref(blk));
